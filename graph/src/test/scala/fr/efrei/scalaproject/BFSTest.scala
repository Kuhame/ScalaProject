...
class BFSSpec extends AnyFlatSpec with Matchers {
  ...
  "BFS" should "visit all vertices in a connected graph" in {
    val graph = DirectedGraph[Int]()
      .addEdge(1, 2)
      .addEdge(2, 3)
      .addEdge(3, 4)

    val result = BFS.bfs(graph, 1)
    result should contain allOf (1, 2, 3, 4)
  }

  it should "handle disconnected graphs correctly and not go trough the vertices it can not reach" in {
    val graph = DirectedGraph[Int]()
      .addEdge(1, 2)
      .addEdge(3, 4)

    val resultFrom1 = BFS.bfs(graph, 1)
    resultFrom1 should contain allOf (1, 2)
    resultFrom1 should not contain (3, 4)

    val resultFrom3 = BFS.bfs(graph, 3)
